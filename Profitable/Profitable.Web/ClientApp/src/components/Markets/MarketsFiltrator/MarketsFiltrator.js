...
export const MarketsFiltrator = ({ instrument, onInstrumentChange }) => {
    ...
    const OnChangeMarketType = (marketTypeSelected) => {
        getAllInstrumentsByMarketType(marketTypeSelected).then((responseInstruments) => {
            setState({
    ...
    return (
        <section className={styles.searchSection}>
            ...
            <div className={styles.searchInstrumentPart}>
                ...
                <select
                    className={styles.searchDropdown}
                    onChange={(e) => OnInstrumentListChangeSetInstrument(e.target.value)}
                    value={state.instrumentSelected.tickerSymbol}
                >
                    {state.instruments.map((instr) => (
                        <option key={instr.guid} value={instr.tickerSymbol}>
                            {instr.tickerSymbol}
                        </option>
                    ))}
                </select>
