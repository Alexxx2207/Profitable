...
import { changePosition, getPositionByGuid } from '../../../../../services/positions/positionsService';
        ...
        case 'loadPosition':
            return {
                ...
                values: {
                    ...
                    directionSelected: LongDirectionName.localeCompare(action.payload.direction) === 0 ? LongDirectionName : ShortDirectionName,
                ...
                errors: {
                    ...
                    entryPriceEmpty: createClientErrorObject(state.errors.entryPriceEmpty, isEmptyOrWhiteSpaceFieldChecker.bind(null, action.payload.entryPrice)),
                    exitPriceEmpty: createClientErrorObject(state.errors.exitPriceEmpty, isEmptyOrWhiteSpaceFieldChecker.bind(null, action.payload.exitPrice)),
                    quantityEmpty: createClientErrorObject(state.errors.quantityEmpty, isEmptyOrWhiteSpaceFieldChecker.bind(null, action.payload.quantity)),
                    quantityOverZero: createClientErrorObject(state.errors.quantityOverZero, isEmptyOrWhiteSpaceFieldChecker.bind(null, action.payload.quantity)),
                }
            };
        case 'setContractsCount':
            if(action.payload >= 0) {
                return {
                    ...
                    errors: {
                        ...state.errors,
                        quantityEmpty: createClientErrorObject(state.errors.quantityEmpty, isEmptyOrWhiteSpaceFieldChecker.bind(null, action.payload)),
                        quantityOverZero: createClientErrorObject(state.errors.quantityOverZero, naturalNumberChecker.bind(null, action.payload)),
                    }
                };
            } else {
                return {
                    ...
                    errors: {
                        ...state.errors,
                        quantityEmpty: createClientErrorObject(state.errors.quantityEmpty, isEmptyOrWhiteSpaceFieldChecker.bind(null, '0')),
                        quantityOverZero: createClientErrorObject(state.errors.quantityOverZero, naturalNumberChecker.bind(null, '0')),
                    }
                };
            }
        case 'updateInput':
            return {
                ...state,
                values: changeStateValuesForControlledForms(state.values, action.payload.name, action.payload.value),
                errors: {
                    ...state.errors,
                    [action.payload.name + 'Empty']: createClientErrorObject(state.errors[action.payload.name+'Empty'], isEmptyOrWhiteSpaceFieldChecker.bind(null, action.payload.value)),
                }
            };
...
export const ChangeFuturesPosition = () => {
    ...
    const [state, setState] = useReducer(reducer, {
        ...
        errors: {
            entryPriceEmpty: { text: 'Insert Entry Price', fulfilled: false, type: CLIENT_ERROR_TYPE },
            exitPriceEmpty: { text: 'Insert Exit Price', fulfilled: false, type: CLIENT_ERROR_TYPE },
            quantityEmpty: { text: 'Insert Contracts Quantity', fulfilled: false, type: CLIENT_ERROR_TYPE },
            quantityOverZero: { text: 'Contracts Quantity > 0', fulfilled: false, type: CLIENT_ERROR_TYPE },
            serverError: {
                text: '',
                display: false,
    ...
    useEffect(() => {
        getUserEmailFromJWT(JWT)
            ...
            .then(futuresContracts => setState({
                type: 'loadAllFuturesContracts', 
                payload: futuresContracts
            }));

            getPositionByGuid(recordGuid, positionGuid)
                .then(position => (
                    setState({
                        type: 'loadPosition',
                        payload: position,
            ...
            changePosition(
                ...
                .catch(err => {
                    if (JWT && err.message === 'Should auth first') {
                        setMessageBoxSettings(JWT_EXPIRED_WHILE_EDITING_ERROR_MESSAGE + ' The position was not edited!', false);
                        removeAuth();
                        navigate('/login');
                    } else {
                        setState(state => ({
                ...
                <div className={styles.changeContainer}>
                    <form className={styles.changeForm} onSubmit={onSubmit} >
                        ...
                        <div className={styles.formGroup}>
                            ...
                            value={state.values.allContracts.find(el => el.name === state.values.chosenContract.name)?.name}>
                                {state.values.allContracts.map((futureContract, index) =>
                                    <option className={styles.selectContractOption} key={futureContract.guid} value={futureContract.name}>{futureContract.name}</option>
                                )}
                            </select>
