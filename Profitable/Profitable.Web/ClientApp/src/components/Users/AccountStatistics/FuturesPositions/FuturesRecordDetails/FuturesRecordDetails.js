...
export const FuturesRecordDetails = () => {
    ...
    const addPositionButtonClickHandler = (e) => {
        navigate(
            `/users/${searchedProfileEmail}/positions-records/futures/${recordGuid}/create-position`
        );
    };
                ...
                <tbody>
                    {state.positions.reverse().map((position, index) => (
                        <tr key={index}>
                            ...
                            <td>{position.contractName}</td>
                            {position.direction.localeCompare(ShortDirectionName) === 0 ? (
                                <td className={styles.textColorRed}>{position.direction}</td>
                            ) : (
                                <td className={styles.textColorGreen}>{position.direction}</td>
                            )}
